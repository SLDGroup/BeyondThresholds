	...
	for seed_i,seed in enumerate(seeds):
		...
		for user_i,user in enumerate(users):
			...
			for bp_i,bp in enumerate(body_parts):
				...
				# create the datasets
				train_loader,val_loader,test_loader = load_rwhar_person_dataset(BATCH_SIZE,train_users,test_users,[bp])
			...
			# create the datasets
			train_loader,val_loader,test_loader = load_rwhar_person_dataset(BATCH_SIZE,train_users,test_users,body_parts)

			# init models
			model = RWHARNet(3*len(body_parts))
			opt = torch.optim.SGD(model.parameters(),lr=LR,momentum=MOMENTUM,weight_decay=WD)
