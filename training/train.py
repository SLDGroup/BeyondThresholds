...
def validate_ens_bps(models, val_loaders, device, loss_fn):
    for model_i,model in enumerate(models):
        models[model_i].eval()
        models[model_i] = model.to(device)
    ...
    with torch.no_grad():
        ...
        for vl in val_loaders:
            iters.append(iter(vl))

        # go prediction for each batch
        for batch_idx in range(len(val_loaders[0])):
            preds = []

            # go through each model's data loader
            ...
            # parse the output for the prediction
            prediction = pred.argmax(dim=1).to('cpu')
