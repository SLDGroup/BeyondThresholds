...
users = [1,2,3,4]# exclude users 2 and 6 due to missing data
seeds = [123,456,789]
logging_prefix = "opportunity"
...
for seed_i,seed in enumerate(seeds):
	...
	for user_i,user in enumerate(users):
		...
		# sensor_bin_counts = {i: np.zeros(n_bins) for i in range(len(body_parts))}
		...
		for sensor in range(len(body_parts)):
			...
			if bin_boundaries.shape[0] == 10:
				bin_boundaries = torch.cat([bin_boundaries,torch.tensor([1])])
			...
			print(sensor_bin_marks[sensor].shape)
		...
		for ui,(ub,ef) in enumerate(zip(utilization_budgets,ensemble_fractions)):
			...
			for sensor in range(len(body_parts)):
				...
				# print(f"\t\t\ts{sensor}: best threshold_U policy: {best_policy_U}")
			...
			with open(f"opportunity_accs_array_{n_bins}_bins___U.pkl", 'wb') as file:
		...
		plt.ylabel("Accuracy")
		plt.legend()
		plt.savefig(f"opportunity__{seed}_{test_users}.png",dpi=200)
...
with open(f"opportunity_accs_array_{n_bins}_bins_U.pkl", 'wb') as file:
