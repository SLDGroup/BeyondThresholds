...
users = [1,3,4,5,7,8,9,10,11,12,13,14,15] # exclude users 2 and 6 due to missing data
seeds = [123,456,789]
logging_prefix = "rwhar"
...
policies = ['random','threshold','optimal','optimal_U','optimal_1','optimal_5']#,'threshold_U']
...
for seed_i,seed in enumerate(seeds):
	...
	for user_i,user in enumerate(users):
		...
		for sensor in range(len(body_parts)):
			...
			for bin_lower, bin_upper in zip(sensor_bin_lowers[sensor], sensor_bin_uppers[sensor]):
				...
				if prop_in_bin.item() > 0:
					...
					# sensor_avg_conf_bins[sensor][i] = sensor_avg_confidence_in_bin
				i+=1
		...
		for ui,(ub,ef) in enumerate(zip(utilization_budgets,ensemble_fractions)):
			print(f"\t\t\tutilization budget: {ub}, {ef}")
			...
			for sensor in range(len(body_parts)):
				...
				# print(max_thresh)
				# print((ordered_confs<=max_thresh).float().mean())
				...
				for th in search_thresh:
					try:
						...
						if exp_acc_ > max_exp_acc:
							...
							best_policy = th
						# print(th,alpha_e,thresh_idx,exp_acc_)
					except:
						continue
						# print(th,alpha_e,(ordered_confs<=th).nonzero().view(-1))
				...
				print(f"\t\t\ts{sensor}: best threshold policy: {best_policy}")
				print(f"\t\t\ts{sensor}: best optimal policy: {sensor_opt_policy[sensor]}")
				print(f"\t\t\ts{sensor}: best optimal_U policy: {sensor_opt_policy_U[sensor]}")
				print(f"\t\t\ts{sensor}: best optimal_1 policy: {sensor_opt_policy_1[sensor]}")
				print(f"\t\t\ts{sensor}: best optimal_5 policy: {sensor_opt_policy_5[sensor]}")
			...
			# print(len(sensor_order),calib_amt)
			for idx,sensor in enumerate(sensor_order):
				...
				count += 1
			print(f"exec time: {time.time()-st}, {count}")
			'''------------------------------------------------------------------'''
			...
			print(f"\t\t\t\trand_f1: {f1_score(rand_lab,rand_pred,average='macro')}")
			rand_accs.append(f1_score(rand_lab,rand_pred,average='macro'))

			print(f"\t\t\t\tthresh_f1: {f1_score(thresh_lab,thresh_pred,average='macro')}")
			threshold_accs.append(f1_score(thresh_lab,thresh_pred,average='macro'))

			print(f"\t\t\t\topt_f1: {f1_score(opt_lab,opt_pred,average='macro')}")
			optimal_accs.append(f1_score(opt_lab,opt_pred,average='macro'))
			print(f"\t\t\t\topt_f1_U: {f1_score(opt_lab_U,opt_pred_U,average='macro')}")
			optimal_accs_U.append(f1_score(opt_lab_U,opt_pred_U,average='macro'))

			print(f"\t\t\t\topt_f1_1: {f1_score(opt_lab_1,opt_pred_1,average='macro')}")
			optimal_accs_1.append(f1_score(opt_lab_1,opt_pred_1,average='macro'))

			print(f"\t\t\t\topt_f1_5: {f1_score(opt_lab_5,opt_pred_5,average='macro')}")
			...
			# accs_array2[seed_i][user_i][3][ui] = f1_score(thresh_lab_U,thresh_pred_U,average='macro')
			...
			with open(f"rwhar_accs_array_{n_bins}_bins___U.pkl", 'wb') as file:
				pickle.dump(accs_array2, file)

			for other_sensor in range(len(body_parts)):
				curr_util = (sum(rand_sel == -1) + sum(rand_sel == other_sensor))/len(rand_sel)
				print(f"\t\t\t\tsensor: {body_parts[other_sensor]}, rand_util: {curr_util}, {sum(rand_sel == -1) + sum(rand_sel == other_sensor)}/{len(rand_sel)}")
				rand_utils.append(curr_util)

				curr_util = (sum(thresh_sel == -1) + sum(thresh_sel == other_sensor))/len(thresh_sel)
				print(f"\t\t\t\tsensor: {body_parts[other_sensor]}, thresh_util: {curr_util}, {sum(thresh_sel == -1) + sum(thresh_sel == other_sensor)}/{len(thresh_sel)}")
				threshold_utils.append(curr_util)

				curr_util = (sum(opt_sel == -1) + sum(opt_sel == other_sensor))/len(opt_sel)
				print(f"\t\t\t\tsensor: {body_parts[other_sensor]}, opt_util: {curr_util}, {sum(opt_sel == -1) + sum(opt_sel == other_sensor)}/{len(opt_sel)}")
				optimal_utils.append(curr_util)
				curr_util_U = (sum(opt_sel_U == -1) + sum(opt_sel_U == other_sensor))/len(opt_sel_U)
				print(f"\t\t\t\tsensor: {body_parts[other_sensor]}, opt_util_U: {curr_util_U}, {sum(opt_sel_U == -1) + sum(opt_sel_U == other_sensor)}/{len(opt_sel_U)}")
				optimal_utils_U.append(curr_util_U)

				curr_util_1 = (sum(opt_sel_1 == -1) + sum(opt_sel_1 == other_sensor))/len(opt_sel_1)
				print(f"\t\t\t\tsensor: {body_parts[other_sensor]}, opt_util_1: {curr_util_1}, {sum(opt_sel_1 == -1) + sum(opt_sel_1 == other_sensor)}/{len(opt_sel_1)}")
				optimal_utils_1.append(curr_util_1)

				curr_util_5 = (sum(opt_sel_5 == -1) + sum(opt_sel_5 == other_sensor))/len(opt_sel_5)
				print(f"\t\t\t\tsensor: {body_parts[other_sensor]}, opt_util_5: {curr_util_5}, {sum(opt_sel_5 == -1) + sum(opt_sel_5 == other_sensor)}/{len(opt_sel_5)}")
		...
		plt.ylabel("Accuracy")
		plt.legend()
		plt.savefig(f"rwhar__{seed}_{test_users}.png",dpi=200)
			# # exit()
...
with open(f"rwhar_accs_array_{n_bins}_bins_U.pkl", 'wb') as file:
